...
type Master struct {
	...
	*FileFetcher
	...
	// map file to worker address
	mapFileAddressByFile    map[string]string
	reduceFileAddressByFile map[string]string

	// record address of fail workers
	FailWorker    map[string]string
	// map files stored in worker
	FilesOfWorker    map[string] []string
	// indexes of tasks done in worker
	TaskIndexesOfWorker    map[string] []int

	// HangTaskNumber map[string]int
