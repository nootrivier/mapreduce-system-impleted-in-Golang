...
func (wk *Worker) DoTask(arg *DoTaskArgs, reply *TaskReply) error {
	...
	case reducePhase:
		...
		for file, address := range arg.MapFileAddressByFile {
			err := wk.pullFile("Worker", address, file)
			if err != nil { 
				reply.MapOrReduceFail = 1
		...
		files := doReduce(arg.JobName, arg.TaskNumber, mergeName(arg.JobName, arg.TaskNumber), arg.NumOtherPhase, wk.Reduce)
